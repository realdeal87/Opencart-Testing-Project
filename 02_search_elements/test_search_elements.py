...
def test_search(browser_driver):
    """Поиск продуктов Canon, успешное добавление найденного продукта в отслеживаемые"""
...
def test_card(browser_driver):
    """Переход к карточке iPhone, добавление 3-х штук в корзину"""
    ...
    browser_driver.find_element(*ProductCard.input_qty).send_keys("3")
    browser_driver.find_element(*ProductCard.button_cart).click()


def test_pictures(browser_driver):
    """Переход к карточке MacBook, просмотр всех фото, добавление в корзину, открытие корзины"""
    products = browser_driver.find_elements_by_xpath(MainPage.Promo.image_link)
    products[0].click()
...
def test_admin_panel(browser_driver, url):
    """Вход в панель администратора с невалидными данными, получение алертинга"""
